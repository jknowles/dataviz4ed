...
```{r effplot1, echo=FALSE, out.width='800px'}
...
```{r modelsims, echo=FALSE}

library(devtools)
library(datasynthR)
library(arm)
dat1 <- genNumeric(1000, 6, rho=0.7)
dat1 <- as.data.frame(dat1)
names(dat1) <- c("y", "x1", "x2", "x3", "x4", "x5")

m1 <- glm(y ~ x1 + x2, data=dat1)
s <- sim(m1, 100)

yhat <- coef(s)
yhat.s <- yhat[,1] + 2 * yhat[,2] + -3 *yhat[,3]

yhat <- cbind(yhat, yhat.s)
