...
```{r plot1.1,out.width="350px",out.height="225px",echo=FALSE}
qplot(hp,mpg,data=mtcars)+theme_dpi()
...
```{r meanplot4, echo=FALSE,out.width='750px',out.height='480px'}
ggplot(schooldat[schooldat$schid<25 & schooldat$grade==3,],
       aes(x=factor(schid),ymin=meanR-sdR,ymax=meanR+sdR,
                     color=perminority))+geom_linerange(aes(alpha=0.7))+
  geom_point(aes(y=meanR,size=count))+
...
```{r echo=FALSE}
library(partykit)
mypar<-ctree_control(testtype="Bonferroni",mincriterion=0.99,minsplit=1000,
                     teststat="max",maxdepth=4)

mymod<-ctree(readSS~race+female+disab+sch_fay+dist_fay+attday,
             data=subset(student_long,grade==4),control=mypar)
...
```{r plot2, fig.height=5.5, fig.width=8.5,fig.align="center",out.width="350px",out.height="240px",echo=FALSE}
...
```{r studentexample,echo=FALSE,out.width='4200px',out.height='350px'}
qplot(attday,readSS,
